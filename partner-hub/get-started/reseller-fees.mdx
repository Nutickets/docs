...
2. **Plan fees** — fees defined on a [plan](/partner-hub/get-started/plans-and-packages). Companies assigned to a plan can inherit its fees automatically.
...
A [plan](/partner-hub/get-started/plans-and-packages) can have its own set of default fees. When a company is set to use plan fees, its fees are synced from the plan automatically. Changes to the plan's fees are reflected on the company.
...
For companies using [custom currencies](/core-platform/epos/custom-currencies), only the fixed fee amount is used — percentage-based calculations are not applied. The fee falls back to the company's default currency configuration, using just the fixed amount.
...
<Note>
A [limitation](/partner-hub/get-started/feature-flags#limitations) exists to prevent companies from absorbing fees: **Disallow absorbing item fees**. When enabled, companies cannot hide reseller or processing fees in their pricing.
</Note>

## Related

- [Plans & Packages](/partner-hub/get-started/plans-and-packages) — define fee templates for groups of companies
- [Default System Fees](/core-platform/fees/default-system-fees) — how fees appear to companies
- [Blended Fees](/core-platform/fees/blended-fees) — detailed explanation of blended fee behaviour
- [Feature Flags](/partner-hub/get-started/feature-flags) — fee-related limitations and capabilities
