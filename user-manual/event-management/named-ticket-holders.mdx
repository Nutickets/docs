...
Named ticket holders let event organisers attach an attendee's name to a ticket without changing who owns it. This is different from [item transfers](/user-manual/event-management/item-transfers), which reassign the ticket entirely — named holders simply record who will be attending.
...
The feature is configured per event with separate controls for the ticket shop and [box office](/user-manual/box-office/introduction).
...
<Note>
If a ticket with a named holder is later [transferred](/user-manual/event-management/item-transfers) to another person, the named holder is automatically cleared. The new owner can then set a new name if the feature is enabled.
</Note>
...
Named holders integrate with [access control](/user-manual/event-management/access-control) scanning. When a ticket is scanned at entry, the system displays the attendee name — allowing door staff to verify the person presenting the ticket matches the named holder.
...
Attendance [reports](/user-manual/reporting/fundamentals) also include the named holder, making it straightforward to audit who attended versus who was expected.
...
<CardGroup cols={2}>
  ...
  <Card title="Data Capture" icon="rectangle-list" href="/user-manual/data-capture/introduction">
    Collect additional attendee information beyond just their name.
  </Card>
  <Card title="Checkout" icon="credit-card" href="/user-manual/ticket-shop/checkout">
