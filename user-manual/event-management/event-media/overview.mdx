...
Text and shape colours are chosen from [colour presets](/user-manual/event-management/event-media/custom-pdfs#colour-presets) that include your brand colours, and status fields such as **Verified resale** print as [badges](/user-manual/event-management/event-media/custom-pdfs#resale-and-refund-protection-badges) only on the tickets they apply to.

For detailed information on creating templates, paper sizes, and available fields, see [Custom PDFs](/user-manual/event-management/event-media/custom-pdfs).
...
<Note>
The settings page also controls [mobile wallet pass](/user-manual/event-management/event-media/mobile-wallets) defaults and whether mobile wallet downloads are enabled.
</Note>
...
4. If starting from a blank template, choose a paper size (see [Paper sizes](/user-manual/event-management/event-media/custom-pdfs#paper-sizes))
...
The API provides endpoints to [retrieve event media templates](/api-reference/event-media/retrieve-defined-event-media) and [generate download links](/api-reference/event-media/retrieve-download-links-for-specific-event-media) for programmatic access to ticket PDFs and passes.
