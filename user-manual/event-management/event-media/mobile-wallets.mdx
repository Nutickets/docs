...
Mobile wallet designs let you create branded digital passes for your customers' tickets. Customers can save their tickets directly to **Apple Wallet** on their iPhone, **Samsung Wallet** on their Galaxy device, or — for events with [secure barcodes](/user-manual/event-management/secure-barcodes) — view them as a **digital pass** in their account.

Wallet passes are an alternative to [PDF tickets](/user-manual/event-management/event-media/custom-pdfs), not a replacement. Both can be available simultaneously, giving customers the choice of how they want to access their tickets.
...
**Automatic background colour:** Enable **Automatic background colour** to colour each pass from your seating plan. When the ticket's seat has a [tag](/user-manual/seating-plans/editor#gates-stairs-stands-tags) set to **Show as ticket info** that has a colour, the pass background uses that tag's colour and its labels switch to white — useful for events with colour-coded sections. Tickets without such a tag, including unseated tickets, use your shop's [colour theme](/user-manual/cms/theme-and-branding#colour-theme) instead. While the setting is on, the **Background colour** picker is hidden, and the designer preview and the template's thumbnail on the event media listing show your colour theme.
...
- **Event name** — Displayed prominently; on Apple Wallet passes the main field pairs the event name with the ticket name
- **Date and time** — Event start date and time
- **Venue** — Venue name
- **Seat details** — Gate, row, seat, and other location details for tickets on a [seating plan](/user-manual/event-management/seated-events) (see [Seat and Entry Details](#seat-and-entry-details))
- **Customer name** — The ticket holder's name
- **QR code** — Scannable barcode for entry

The back of the pass can include additional details such as your organisation name, the order reference, and the full venue address.

### Seat and Entry Details

<Tip>Clearer row, seat, and gate display in Release 48</Tip>

Every seat detail on a pass carries its own label, so customers and entry staff can tell the gate, row, and seat apart at a glance. Only the details that apply to the ticket are shown, from **Gate**, **Stair**, **Area**, **Stand**, **Block**, **Row**, **Table**, and **Seat** — or **Space** for general admission. The values come from the seating plan: its areas, blocks, rows, tables, and seat labels, plus the [gates, stairs, and stands](/user-manual/seating-plans/editor#gates-stairs-stands-tags) assigned to each seat.

| Pass | How seat details are shown |
|------|---------------------------|
| **Apple Wallet** | The front groups the details into up to three labelled fields — **Gate - Stair**, **Area - Stand - Block**, and **Row - Table - Seat** — so a ticket might show *Row - Seat* with the value *F - 12*. The back lists each detail on its own line. |
| **Samsung Wallet** | The gate uses Samsung Wallet's own entrance field, and the seat on seated tickets uses its seat number field. The remaining details — including the space, for general admission — are listed individually with their labels. |
| **Digital pass** | The front lists each detail in a labelled grid. When the design uses a strip image, the same details also appear in large type in a band across the image. |

For general admission spaces, Block is left out — except on the back of an Apple Wallet pass, where it is still listed when it differs from the space name. On an Apple Wallet pass for a ticket with no seat, the **Location** field (the venue name) takes the place of the seat details on the front.

<Note>
In the digital pass band, a leading label is removed from the value so it isn't repeated — for example, a gate named *Gate 3* appears as *3* under **Gate**. Values of 30 characters or more are left out of the band but still appear in the grid below.
</Note>
...
Apple Pass templates follow the same [assignment hierarchy](/user-manual/event-management/event-media/overview#assigning-templates) as printable templates. The platform resolves which pass design to use by checking:
...
Unlike [PDF downloads](/user-manual/event-management/event-media/custom-pdfs#secure-barcodes-and-pdf-downloads), Apple Wallet passes **fully support secure barcodes**. Because the pass is a live digital document on the customer's phone, the barcode can refresh dynamically. There is no need to convert to static barcodes when using wallet passes.
