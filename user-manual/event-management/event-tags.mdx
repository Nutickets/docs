...
- Tags are labels that can be attached to events to categorise and organise them. They are shared across events and [articles](/user-manual/cms/articles).
- Tags can optionally belong to a **tag category**. Categories may be marked as **mandatory**, requiring organisers to assign at least one tag from that category during [event creation](/user-manual/event-management/introduction#creating-events).
- Tags are used to filter events in [event search](/user-manual/ticket-shop/browsing-and-search) and to automatically populate [event listing blocks](/user-manual/ticket-shop/browsing-and-search) with matching events.
- Tag names support [multiple languages](/user-manual/core/translations) and are limited to 50 characters per translation.
...
Tags are available as a filter in [event search](/user-manual/ticket-shop/browsing-and-search). Customers can filter events by selecting one or more tags, which narrows the listing to events matching any of the selected tags.
...
[Event listing blocks](/user-manual/ticket-shop/browsing-and-search) can be configured to display events with specific tags. Events are automatically included based on their tag assignments, removing the need for manual event selection.
...
Tags can be imported via CSV from the [data imports](/user-manual/data-imports/overview) feature. The import supports:
