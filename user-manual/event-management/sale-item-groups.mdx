...
- Tickets that belong to a [timeslot](/user-manual/event-management/timeslots) cannot be placed in a group.
...
If the event belongs to a [schedule](/user-manual/event-management/schedules), you can apply the new group to other events in the same schedule during creation.
...
<Note>
  ...
  [availability](/user-manual/event-management/capacity-controls).
</Note>
...
When an event belongs to a [schedule](/user-manual/event-management/schedules), groups can be synchronised across events in that schedule:
...
See [data imports](/user-manual/data-imports/overview) for details.
