...
Within **Events** results, an event whose name exactly matches the search term is listed first, followed by events whose names start with it, then other matches by relevance. Matching ignores upper and lower case.
...
| **Tags** | Filter events by category or type (uses your configured [event tags](/user-manual/event-management/event-tags)) |
...
| **Most Relevant** | Ordered by search match score. Only appears when the customer has an active search query. If a search is active and no sort has been chosen, events are ranked by name first — exact matches, then names that start with the search term — and then by search match score. | Shop and Box Office |
...
- [Event Tags](/user-manual/event-management/event-tags) — Create and manage the tags customers use to filter events
- [Homepage](/user-manual/ticket-shop/homepage) — Configure event listing blocks on shop pages
