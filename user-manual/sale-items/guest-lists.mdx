...
| **Approval** | Not required (unless [item approvals](/user-manual/sale-items/item-approvals) are enabled) | Always requires approval |
...
- **Display settings** — control visibility across [sales channels](/user-manual/information/sales-channels)
- **Scanning settings** — configure barcode validity windows and grace periods for [access control](/user-manual/event-management/access-control)
- **Print and wallet settings** — control whether guest list entries generate printable tickets or [mobile wallet](/user-manual/event-management/event-media/mobile-wallets) passes
...
Customers add guest list items to their basket like any other item, selecting the number of guests they want to register. During [checkout](/user-manual/ticket-shop/checkout), they complete any required [data capture](/user-manual/data-capture/introduction) questions.
...
- Tickets or passes become available for download (subject to any [deferred download](/user-manual/event-management/deferred-downloads) settings)
...
Guest list items integrate with the [access control](/user-manual/event-management/access-control) system. When attendees are scanned at the event, the system records:
...
<CardGroup cols={2}>
  <Card title="Item Approvals" icon="clipboard-check" href="/user-manual/sale-items/item-approvals">
  ...
  <Card title="Invoice Payments" icon="file-invoice" href="/user-manual/payments/invoice-payments">
