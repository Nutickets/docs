...
| **[On-sale dates](/user-manual/sale-items/core-concepts#on-sale-dates)** | When the donation is available. |
| **[Sales channels](/user-manual/sale-items/core-concepts#sales-channels)** | Where the donation can be made (shop, box office). Variants can be restricted to specific channels. |
| **[Visibility](/user-manual/sale-items/core-concepts#visibility--display)** | Hide when off sale, custom sale badges. |
...
1. Turn on **Giftaid available** in the [donation settings](#donation-settings) and set the **Giftaid Company Amount (%)** (e.g. 25%).
2. Mark individual donation items as **Attracts Gift Aid**.

Enabling Gift Aid sets the **Full name** and **Address** [built-in customer fields](/user-manual/data-capture/introduction#built-in-customer-fields) to **Required**, because both appear on every Gift Aid declaration. While Gift Aid is enabled, these fields can't be made optional or hidden — trying to do so shows: *"This field must be set to mandatory and cannot be hidden as a requirement of Gift Aid"*. The **Title** field isn't required; when a customer has provided a title, it's included with their name on the declaration.
...
| Setting | Description |
|---------|-------------|
| **Giftaid available** | Enables Gift Aid collection on donations for a charity. The **Full name** and **Address** customer fields become required (see [Setup](#setup)). |
| **Giftaid Company Amount (%)** | The Gift Aid rate (0–60%). |
...
The platform integrates with [JustGiving](/user-manual/integrations/justgiving) to support event-level fundraising pages. When configured, customers can create personal or team fundraising pages linked to the event's charity after completing their purchase. This is separate from the direct donation feature but complements it by enabling peer-to-peer fundraising.

## Restrictions

Donations have a narrower set of capabilities compared to other sale item types:

- Cannot be [bundled](/user-manual/sale-items/bundles) or used as [add-ons](/user-manual/sale-items/add-ons).
- Cannot belong to [timeslots](/user-manual/event-management/timeslots) or [sale item groups](/user-manual/event-management/sale-item-groups).
- Cannot be scanned, downloaded, or shared.
- Cannot have [prerequisites](/user-manual/sale-items/core-concepts#prerequisites--restrictions) or attract [discounts](/user-manual/basket-codes/discount-codes).
- No [stock management](/user-manual/sale-items/core-concepts#stock-management) — donations have unlimited availability.
