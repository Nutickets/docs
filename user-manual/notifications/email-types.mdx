...
<Note>
All emails listed here can have their content customised through the website text manager. See [Customising Emails](/user-manual/notifications/customising-emails) for details.
</Note>
...
Sent to the purchaser when an order is completed successfully. Contains the order reference, a summary of purchased items grouped by event, download links for tickets and QR codes, and calendar links. Tickets bought through [resale](/user-manual/resale/introduction) are marked with a **Verified resale** badge beneath the item name — see [The Verified Resale Badge](/user-manual/resale/selling-and-buying#the-verified-resale-badge).
...
| **Conditions** | Not sent if confirmation emails are disabled for the channel (see [Disabling Confirmation Emails](/user-manual/notifications/email-settings#disabling-confirmation-emails)) |

<Note>
If an order contains items from multiple events and the **Send a separate confirmation email per event** setting is enabled, the customer receives a separate confirmation email for each event rather than a single combined email.
</Note>
...
Sent automatically before a reservation's payment deadline while the customer still has an outstanding balance. There are three versions — for payment links, deposit balances, and invoices — each stating the amount outstanding and the deadline, with a **Pay now** button.

| Detail | Value |
|--------|-------|
| **Triggered by** | Automatic — a configurable time before a payment link expires, a deposit balance is due, or an invoice is due |
| **Recipient** | The email address on the order |
| **Subject** | *Reminder: your payment link with [Company] is expiring soon*, *Reminder: your balance with [Company] is due soon*, or *Reminder: your invoice with [Company] is due soon* |
| **Conditions** | Only sent if [payment reminders](/user-manual/notifications/payment-reminders) are turned on for the payment method. Each reservation is reminded once automatically |
...
| **Conditions** | Only sent if [automatic reminder emails](/user-manual/notifications/email-settings#automatic-event-reminder-emails) are enabled |
...
### Resale Seller Notifications

<Tip>Listing closure emails added in Release 47</Tip>

Sent to customers who list tickets for [resale](/user-manual/resale/introduction), to keep them updated on their listing. Each has a **View resale listing** button.

| Email | Subject | Triggered by |
|-------|---------|--------------|
| Listing active | *Your resale listing is active!* | The listing goes live and becomes available to buyers |
| Items sold | *Congratulations! Your tickets have been sold* | One or more items in the listing are bought |
| Listing expired | *Your resale listing has expired* | The listing reaches the end of its sale time with items still unsold |
| Listing closed | *Your resale listing has been closed* | The seller or an admin closes the listing while items are still unsold |

| Detail | Value |
|--------|-------|
| **Recipient** | The seller |
| **Conditions** | Only sent when resale seller [notifications](/user-manual/resale/configuration#notifications) are enabled for the event. The expired and closed emails list the unsold items and aren't sent if everything in the listing sold |
...
| **Recipient** | The email addresses configured in the event's [email preferences](/user-manual/notifications/email-settings#purchase-summary-emails) |
...
<Note>
An enriched variant is available that includes additional detail such as billing address, customer address, and data capture responses. Enable this via **Enriched purchase notifications** in company settings.
</Note>
...
| **Recipient** | The email addresses configured in **Event approval emails** in General settings |
| **Subject** | *Event approval request: [event name]* |
| **Conditions** | Only sent if approval email addresses are configured |

See [Event Approvals](/user-manual/event-management/event-approvals) for the full approval workflow.
...
Sent after a bulk refund operation completes, summarising the results: the orders that were refunded (with the amount and payment gateway) or cancelled, any refunds still being confirmed by the payment provider, and any orders that failed. See [Bulk Refunds](/user-manual/order-management/refunds-and-cancellations#bulk-refunds).
