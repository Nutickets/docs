...
| **Company BCC email** | A blind copy of every order and customer email — the emails recorded in your [email logs](/user-manual/notifications/email-logs) — is sent to this address, for example so an outbound email tracking tool can see them. Only available when enabled by your account manager <Tip>Added in Release 47</Tip> |
...
When a from email, reply-to email, or BCC email is configured, a **Send test email** button appears. Clicking it sends a test email to your own email address — with a blind copy to the BCC address, if one is set — to confirm the configuration is working. The test email confirms whether the addresses are correctly verified for sending.
...
2. Click **Email settings** (in the quick actions)
...
Enable this at the company level in **Company Settings** with the **Enriched purchase notifications** toggle. This applies to all events under the company.

### Enriched Order Confirmations

A similar enrichment is available for customer-facing order confirmation emails. When **Enriched order confirmation emails** is enabled in company settings, order confirmation emails sent to customers include address details and data capture responses.

---

## Confirmation Email Splitting

By default, when a customer purchases items from multiple events in a single order, they receive one confirmation email covering all items. The **Send a separate confirmation email per event** setting in company settings changes this behaviour so the customer receives a separate confirmation email for each event.
...
<Note>
Reminders for customers who haven't yet paid for a payment link, deposit balance, or invoice are configured separately, alongside each payment method. See [Payment Reminders](/user-manual/notifications/payment-reminders).
</Note>

### Enabling at the Company Level

1. Navigate to **Company Settings**
2. Find **Automatic event reminder emails**
3. Enable the toggle
4. Set **Send reminders this long before the event** — this determines how far in advance of the event start time the reminder is sent (configurable in hours, days, or weeks)
...
The subject and body of the reminder email can be customised through the website text manager. From the event's **Email settings** modal, click **Customise reminder email** to go directly to the relevant text editing page.

See [Customising Emails](/user-manual/notifications/customising-emails) for more on editing email content.

---

## Event Approval Emails

When using the [event approval workflow](/user-manual/event-management/event-approvals), email addresses can be configured to receive notifications when approval is requested.

1. Navigate to **Company Settings**
2. Find **Event approval emails**
...
- [Email Types](/user-manual/notifications/email-types) — Complete reference of every email
