...
<Note>
Blocking a device does not revoke any API tokens. The device itself is blocked — if the same token is used from a different device, those requests will still work. To fully revoke access, also revoke the associated [API token](/user-manual/utilities/api-token-management).
</Note>
