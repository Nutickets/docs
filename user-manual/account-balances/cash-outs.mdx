...
| **Offline** | Nothing — the platform doesn't return the money. The cash out is recorded against top ups paid by a wider set of payment methods, including in-person top-up purchases | Admin-initiated cash outs from the admin panel |
...
4. Select the cash-out type:
   - **Cash out online (refund)** — resets the balance and attempts to return the money to the customer's original payment method(s). Covers top ups paid via online payment methods
   - **Cash out offline** — resets the balance, but no money is returned to the customer by the platform. Covers top ups paid via any payment method (broader eligibility)
...
### When a Balance Can't Be Cashed Out

<Tip>Cash-out explanations added in Release 47</Tip>

Once you pick a cash-out type, the panel explains why if there's nothing that type can return:

| Message | When it appears |
|---------|-----------------|
| *No funds are available for the selected cash-out type. This balance can be returned using the offline option instead. Switch the cash-out type to continue.* | The selected type has nothing to return, but the other type does — for example, a top up paid in cash or by bank transfer can be cashed out offline but not online. The message names whichever type can be used. |
| *This balance cannot be cashed out. Only unused cashable top-ups can be returned. Refunds to balance, manual or offline adjustments, and spent top-ups cannot be cashed out.* | The customer has a balance, but none of it can be cashed out with either type, because it didn't come from unspent top ups. See [Refundable Balance](#refundable-balance). |
| *Available balance to cash out is empty.* | The customer's balance is empty. |

The help text beside the cash-out type options states the same rule, so staff can see before choosing a type which balances can be returned.

Cash outs can also be processed programmatically via the [Create Wallet Cashout](/api-reference/wallets/create-wallet-cashout) API endpoint.
...
<CardGroup cols={2}>
  <Card title="Top Ups" icon="circle-plus" href="/user-manual/account-balances/top-ups">
