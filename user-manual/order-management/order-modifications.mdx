...
Once a modification is complete, the replacement order is the active order and can be [refunded](/user-manual/order-management/refunds-and-cancellations) like any other completed order. The original order is cancelled — refund actions are only available on the replacement order.
...
<Note>
For replacement orders paid with a combination of previous order balance and an additional payment, the refund amount is distributed proportionately across both payment components — the same way [split payment](/user-manual/payments/split-payments) refunds are allocated.
</Note>
...
- [Refunds And Cancellations](/user-manual/order-management/refunds-and-cancellations)
