...
All sharing settings support [translations](/user-manual/core/translations). Use the language switcher on each field to enter different titles, descriptions, and images for each of your configured languages. The platform serves the version matching the customer's current language.
...
<Tip>Clean canonical URLs added in Release 47</Tip>

The platform generates:

- A **canonical URL** for each page — its clean address on your shop domain, without a `?language=` parameter. Whichever language a visitor is browsing in, search engines are pointed to the same preferred address, so each page is indexed once rather than once per language.
- **Alternate language links** (`hreflang`) for each of your configured languages, so search engines know about all language versions of a page. The link for your company's fallback language uses the same clean address as the canonical URL.

If you use language-specific domains (e.g., `fr.example.com` for French), the alternate links for those languages point to those domains. Other languages use a `?language=` parameter.
...
<Tip>Event dates cover content, sale item, and pricing changes in Release 47</Tip>

Each page in the sitemap carries the date it last changed, so search engines can tell when a page is worth crawling again. For events, this is the most recent change to anything shown on the event page — the event itself, its settings, content blocks, images, sale items, or pricing — not just edits to the event's own details.

### Multi-Language Sitemaps

Each URL in the sitemap is the page's clean address — the same one used as its [canonical URL](#canonical-urls-and-language-alternates) — and includes alternate language links for all your configured languages. This helps search engines understand that the same content exists in multiple languages and serve the right version to users in different regions.
...
<Tip>Ticket prices added to event structured data in Release 47</Tip>

The platform automatically generates structured data (JSON-LD, Schema.org) for events and articles, helping search engines understand your content and enabling rich search results like the Google events carousel. This is enabled by default and can be disabled at the company level if needed.

<Accordion title="What structured data is generated">

**Event pages** include: event name, start/end dates, venue name and address (if assigned), organiser name, and ticket prices. This can enable Google to show your events in event-specific search features, including the events carousel and Knowledge Panel.

Ticket prices are added as an offer in your company's default currency, following the event's [pricing display](/user-manual/event-management/event-page-customisations#pricing-display) settings — the lowest-to-highest price range when the page shows both, or a single price when the event has one price or the page shows only the lowest or highest. Events with no priced items, or whose only prices are free while **Hide free items** is turned on, have no offer.

**Article pages** include: headline, header image, publication date, last modified date, and author name.

**Site-level** includes a `WebSite` schema with a `SearchAction`, which can enable a sitelinks search box in Google results — allowing users to search your events directly from the search results page.
...
<CardGroup cols={2}>
  <Card title="Translations" icon="language" href="/user-manual/core/translations">
