...
2. **Conditional tax band mappings** — linking each classification to a specific [tax band](/user-manual/tax/tax-bands)
...
3. For each country classification, select a [tax band](/user-manual/tax/tax-bands) from the dropdown.
...
For conditional tax bands with reverse charge to work accurately, customers may need to provide their VAT number during checkout. When VAT number collection is enabled, customers see an additional step at checkout where they can enter their VAT number for verification. See [Advanced Tax Settings](/user-manual/tax/advanced-tax-settings) for details on enabling VAT number collection.
