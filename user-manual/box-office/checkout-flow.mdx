...
The **Box office guest checkout** company setting controls how customer assignment works. The help text reads: **"Controls how much customer info box office staff need to capture at checkout. Choose to allow guest checkout with no details, ask for a name only (so tickets stay tied to a person), or require name and email."**
...
If an event has [ticket naming](/user-manual/event-management/named-ticket-holders) enabled for box office, a step appears during checkout where staff can assign an attendee name to each ticket. This is controlled by per-event settings:
...
<CardGroup cols={2}>
  ...
  <Card title="Discounts and Overrides" icon="percent" href="/user-manual/box-office/discounts-and-overrides">
