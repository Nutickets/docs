...
Integrations can read how an order was split through the orders API: request the `transactions` include on [Retrieve Order](/api-reference/orders/retrieve-order) or [Retrieve Orders](/api-reference/orders/retrieve-orders). Transactions that belong to the same split payment share a `splitPaymentId`, and `isPayment` separates the money taken from refunds and bookkeeping entries.
...
When an order uses split payments, taxes, platform fees, and processing fees are allocated proportionally across the splits based on each split's share of the total. This ensures that financial records accurately reflect how much of each fee was collected through each payment method.
