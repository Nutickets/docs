...
<Accordion title="Complete status reference">
...
Deferred flows create the order first and collect payment later. Each method follows a similar pattern but with different customer experiences and expiry behaviour. Payment links, invoices with a due date, and [deposit balances](#deposit-payment-flow) can also email the customer an automatic [payment reminder](/user-manual/notifications/payment-reminders) before their deadline.
...
#### Reminding Customers to Pay

<Tip>Automatic reminders added in Release 48</Tip>

To remind customers automatically, set **Send a payment reminder before expiry** in **Settings > Payment > Checkout settings**. The customer is emailed once, a set time before their payment link expires — including customers who opened the link but didn't finish paying. See [Payment Reminders](/user-manual/notifications/payment-reminders) for how the timing works.

To contact a customer yourself, use **Resend payment instructions** on the reservation to send the payment link email again, or resend payment links in bulk from the event dashboard's [bulk email tool](/user-manual/notifications/sending-emails#payment-reminder-emails). Once an automatic reminder has been sent, **Resend deadline email** in the reservation's email log sends the reminder again.
...
Invoice orders can optionally have an expiry date set, controlled by the **default payment period** setting. See [Invoice Payments](/user-manual/payments/invoice-payments) for full configuration details.
...
The remaining balance must be paid before the configured deadline. If the customer does not pay in time, the reservation may be cancelled and items released. See [Deposit Payments](/user-manual/payments/deposit-payments) for full details.
