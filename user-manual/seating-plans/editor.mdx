...
<Note>
The editor needs a browser window at least 768 pixels wide. In a narrower window — on a phone, for example — the editor is hidden and the page shows *"The editor requires a wider window to use. Please resize your browser or use another screen or device."*
</Note>
...
| **Zoom only with Ctrl/Cmd scroll**                 | Requires Ctrl (Windows) or Cmd (Mac) to be held for the mouse wheel to zoom the plan | Letting the page scroll normally when the pointer passes over the plan <Tip>Added in Release 47</Tip> |
...
<Note>
**Zoom only with Ctrl/Cmd scroll** is on by default, including for existing plans. While it's on, scrolling over the plan without holding the key doesn't zoom — instead, an overlay briefly tells you to use Ctrl (Cmd on a Mac) + scroll to zoom. Turn it off to zoom with the mouse wheel alone. The setting is saved with the plan, so it also applies wherever the plan is shown, including the seat selector customers use in the shop.
</Note>
...
| Group          | Available tooling                               |
| -------------- | ----------------------------------------------- |
| **Appearance** | Fill colour, opacity, stroke width/style/colour — colours use [colour presets](#colour-presets) |
| **Geometry**   | Radius (circle), width/height/angle (rectangle), border radius (rectangle) <Tip>Border radius added in Release 48</Tip> |
...
| **Typography**    | Font size, text colour, style, decoration                                           |
| **Text block**    | Automatic size, width/height, horizontal and vertical padding (while automatic size is off), border radius, shrink-to-fit, horizontal/vertical alignment <Tip>Border radius and separate horizontal/vertical padding added in Release 48</Tip> |
| **Box styling**   | Stroke width/style/colour, background colour — colours use [colour presets](#colour-presets) |
| **Linking**       | Add/remove URL link and link target                                                 |
| **Display**       | Angle                                                                               |

### Colour Presets

<Tip>Colour presets added in Release 48</Tip>

The colour settings on shapes (**Fill colour**, **Stroke colour**) and text (**Text colour**, **Stroke colour**, **Background colour**) are dropdowns of presets rather than free colour pickers:

| Option                            | Colour                                                                                                                   |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Transparent**                   | No colour — use it to remove a text background or outline                                                                |
| **Grey - light**, **Grey - dark** | Fixed grey shades                                                                                                        |
| **Black**, **White**              | Black or white                                                                                                           |
| **Brand - 1** to **Brand - 11**   | The shades of your shop's [colour theme](/user-manual/cms/theme-and-branding#colour-theme), from lightest to darkest     |
| **Custom**                        | Opens a colour picker for any hex value, starting from the object's current colour                                      |

A brand shade is saved as a reference to your colour theme rather than as a fixed colour, so plan text and shapes follow any later change to your theme — in the editor and on the plan customers see in the shop. Colours chosen with **Custom** stay fixed. To remove a text background, choose **Transparent**; this replaces the previous **Has background colour** toggle.

Area stroke and fill colours and the plan background colour still use the standard colour picker. The same presets are used in the [event media template designer](/user-manual/event-management/event-media/custom-pdfs#colour-presets).
