...
Discount codes can also be managed via the API. Endpoints are available for creating and updating [fixed value](/api-reference/discount-codes/create-fixed-value-discount-code), [percentage](/api-reference/discount-codes/create-percentage-discount-code), and [Buy X Get Y](/api-reference/discount-codes/create-buy-x-get-y-discount) codes, as well as [retrieving](/api-reference/discount-codes/retrieve-discount-codes) existing codes.
...
| Rule | Purpose |
|------|---------|
| **Maximum number of orders** | Total number of orders that can use this code across all customers |
| **Maximum number of items** | Total number of items that can be discounted across all orders |
| **Maximum orders per customer** | Maximum orders per customer using this code |
| **Maximum items, across orders, per customer** | Maximum items per customer that can be discounted |
| **Maximum orders per customer per event** | Maximum orders per customer per event |
| **Maximum orders per customer per schedule** | Maximum orders per customer per schedule |
...
| **Customer groups** | Restricts the code to customers in specific [customer groups](/user-manual/customer-management/customer-groups). Customer groups must be enabled on your account. |
...
<Tip>New default in Release 47</Tip>

The **Visible in Box Office** setting controls whether a code is available to [box office](/user-manual/box-office/introduction) staff. New discount codes are now **visible in box office by default**, since codes are most often used there. Existing codes are unaffected.

| State | Effect |
|------|---------|
| **Visible in Box Office** enabled | The code appears in the box office discount code dropdown for staff to apply. Toggle **Only show to users with a specific role** to restrict it to a single box office role; leaving the role unset makes it **Visible to all users in box office**. |
| **Visible in Box Office** disabled | The code is not available in box office. If staff try to apply it, they see *"This discount code is not available in box office."* |

To change the setting, open the discount code detail page and toggle **Visible in Box Office** in the eligibility options. Turning it off also clears any box office role restriction.
...
Codes are generated in the background. The parent detail page shows children organised by status: **Active**, **Inactive**, **Disabled**, and **Deleted**. Individual child codes can be deleted or disabled from these lists in the same way as any other code (see [Disabling and Deleting](#disabling-and-deleting)). A child code that hasn't been used is deleted and moves to the **Deleted** tab; one that has been used is disabled instead, so its order history is kept. The same applies to the parent: once any of its child codes has been used, deleting the parent disables it instead.

<Note>
**Add children** is unavailable on a code with **Auto apply** enabled, or on a code that has already been used in an order and has no child codes — create a new parent code instead. Once a code has children, you can add more at any time, even after existing child codes have been used. Expired, cancelled and refunded orders don't count as uses.
</Note>
...
The new code must be unique among your discount codes and can't contain spaces. A child code can only be renamed until it's used. Once it has been used in an order, the pencil icon is no longer shown.
...
- **External voucher codes** — see [External Vouchers](/user-manual/basket-codes/external-vouchers)
...
If a customer is part-way through a purchase with the code — or, for a parent code, with any of its child codes — you're asked to confirm before it's disabled, for example: *"The code 'SUMMER10' is currently in 2 active basket(s). Disabling it will remove it from those baskets before payment. Are you sure?"* Confirming disables the code.
...
The discount code detail page includes **View as Report** and **Export** options. Reports show usage data including order counts, item counts, and discount amounts — see the [Discount Code Reports](/user-manual/reporting/default-reports#discount-code-reports) reference for available fields. For parent codes, reports include data across all child codes.

## Validation Errors

When a discount code cannot be applied, the customer sees a message explaining why. Common reasons include the code being invalid or disabled, usage limits being reached, missing eligible items in the basket, or the code being outside its validity period.

<Accordion title="Full validation error reference">
...
- [Access Codes](/user-manual/basket-codes/access-codes)
...
- [Box Office Introduction](/user-manual/box-office/introduction)
