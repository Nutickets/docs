...
This is labelled **Cross-site tracking pixels & tag manager** in the UI, reached from the **Tracking in iframes** link in your cookies and tracking settings. The embedded shop sends tracking events to the host page, allowing the external site to capture conversion data without needing to add tracking codes to their own shop configuration.
...
Google Analytics shaped events are sent to the host page regardless of the customer's cookie choice, so the host site must apply consent itself before passing the data on. Pixel shaped events are only sent when the pixel is enabled for the event and the customer has accepted all cookies.

For the full list of events, their parameters, and what customer data each one carries, see [Embed Messages](/user-manual/technical/embed-messages). For how to set up an embed, see [Website Integration](/user-manual/technical/iframe-embeds).
...
- [Embed Messages](/user-manual/technical/embed-messages)
- [Google Tag Manager](/user-manual/integrations/google-tag-manager)
- [Checkout](/user-manual/ticket-shop/checkout)
- [Basket](/user-manual/ticket-shop/basket)
