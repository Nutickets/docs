...
Where an embed is required, the shop sends the same e-commerce events listed above to the host page instead, so tracking can run on the host site's own domain. These messages fire whether or not a GTM code is configured on the shop. See [Embed Messages](/user-manual/technical/embed-messages) for the full contract.
