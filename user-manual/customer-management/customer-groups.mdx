...
Customer groups let you segment your customer base and use that segmentation to control who can purchase specific [sale items](/user-manual/sale-items/core-concepts) or apply specific [discount codes](/user-manual/basket-codes/discount-codes). A customer who isn't in a required group won't see the restricted item on the event page and won't be able to apply the restricted discount.
...
<Accordion title="Presale access for previous attendees">
  Create an automatic group that includes customers who purchased tickets to a previous event or schedule. Restrict presale tickets to this group so only returning customers can purchase during the [presale period](/user-manual/event-management/sales-periods).
</Accordion>
...
<Accordion title="Loyalty discounts">
  Create an automatic group for customers who have made a minimum number of purchases within a time window. Restrict a [discount code](/user-manual/basket-codes/discount-codes) to this group so only qualifying customers can apply the discount.
</Accordion>
...
[Discount codes](/user-manual/basket-codes/discount-codes) can be restricted so that only customers in specific groups can apply them. The customer must be logged in and belong to at least one of the selected groups for the discount to be accepted.
...
For bulk operations, use the API to [add](/api-reference/customer-groups/add-customers-to-group) or [remove](/api-reference/customer-groups/remove-customers-from-group) multiple customers at once, or use the [customer data import](/user-manual/data-imports/overview) to set group membership via CSV.
...
<CardGroup cols={2}>
  <Card title="Sale Items" icon="tag" href="/user-manual/sale-items/core-concepts">
    Configure sale items, including customer group restrictions.
  </Card>
  <Card title="Discount Codes" icon="percent" href="/user-manual/basket-codes/discount-codes">
    Create discount codes with optional customer group restrictions.
  </Card>
  <Card title="Sales Periods" icon="clock" href="/user-manual/event-management/sales-periods">
    Control when items go on sale — combine with customer groups for presale access.
  </Card>
  <Card title="Data Imports" icon="file-import" href="/user-manual/data-imports/overview">
    Import customer data and group memberships via CSV.
  </Card>
  <Card title="Managing Customers" icon="user" href="/user-manual/customer-management/managing-customers">
