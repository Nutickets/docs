...
When an event uses a [seating plan](/core-platform/seating-plans/introduction) with holds applied, hold data is displayed directly on the [event dashboard](/core-platform/event-management/introduction). Each hold shows its name, ID, and capacity information — making it easy to check hold usage at a glance without navigating to the seating plan.
