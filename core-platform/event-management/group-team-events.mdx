...
<Note>
Teams cannot be enabled for events that have [season tickets](/core-platform/sale-items/season-tickets). Enabling teams on such an event is blocked with a validation error.
</Note>
...
<Note>
[Box office](/core-platform/box-office/introduction) users with the appropriate permission can bypass minimum and maximum team size validation.
</Note>
...
Team settings can be configured on individual events within a [schedule](/core-platform/event-scheduling/core-concepts). When updating team settings, you can apply the changes to the current event only or mass-update all events in the schedule.
