...
- Tags are used to filter events in [event search](/core-platform/ticket-shop/event-search) and to automatically populate [event listing blocks](/core-platform/ticket-shop/event-listings) with matching events.
...
Tags are available as a filter in [event search](/core-platform/ticket-shop/event-search). Customers can filter events by selecting one or more tags, which narrows the listing to events matching any of the selected tags.
...
[Event listing blocks](/core-platform/ticket-shop/event-listings) can be configured to display events with specific tags. Events are automatically included based on their tag assignments, removing the need for manual event selection.
