...
A venue can also be created inline during event creation when selecting a venue in the event wizard. Venues can also be [created](/api-reference/venues/create-a-venue) and [retrieved](/api-reference/venues/retrieve-venues) via the API.
...
Venues can be assigned default [event media](/core-platform/event-management/event-media/overview) templates. These serve as fallback templates for all events at the venue — if an event does not specify its own media templates, the venue's templates are used.
...
| **[Event media](/core-platform/event-management/event-media/overview)** | Default printable and Apple Pass templates applied to events at the venue.    |
| **[Schedules](/core-platform/event-management/schedules)**              | Multiple events within a schedule can share the same venue.                   |
