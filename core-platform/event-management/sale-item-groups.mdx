...
If the event belongs to a [schedule](/core-platform/event-scheduling/core-concepts), you can apply the new group to other events in the same schedule during creation.
...
<Note>
Group stock is tracked independently. When a ticket in the group is purchased, group stock is deducted. If the group runs out of stock, no further tickets in the group can be sold — even if individual tickets still have [availability](/core-platform/event-management/capacity-controls).
</Note>
...
When an event belongs to a [schedule](/core-platform/event-scheduling/core-concepts), groups can be synchronised across events in that schedule:
