...
Each tier has a **Tier minimum** and **Tier maximum** that set the value range it covers, plus the fee amount to charge within that range. Use **Add price tier** to add more tiers. The first tier starts at zero and the final tier is left open-ended, so the full value range is always covered.
