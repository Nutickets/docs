...
<Accordion title="PayPal">
  ...
  **Settings:**
  ...
  PayPal can attribute payments processed through PPCP to a specific partner using a **Partner Attribution ID** (also called a **BN Code**). This identifier is sent to PayPal on every transaction so PayPal can recognise which partner facilitated the payment.

  The BN Code is part of the platform-level PayPal configuration managed by your account manager, with separate values for demo (sandbox) and live modes. When no BN Code is set for a mode, the default platform BN Code is used, so existing PayPal integrations are unaffected. Contact your account manager if you need a specific Partner Attribution ID applied to your PayPal payments.
</Accordion>
...
For gateways that do not support automatic fee collection (such as Adyen, QuickPay, and Worldpay), platform fees are tracked separately and settled through [invoicing](/core-platform/information/paying-invoices).
...
- [Default System Fees](/core-platform/fees/default-system-fees)
