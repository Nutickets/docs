...
| Setting | Purpose |
|---------|---------|
| **SSO provider name** | The label displayed to customers (e.g. your organisation's name) |
| **Change email URL** | Redirects customers to the external system to change their email |
| **Change password URL** | Redirects customers to the external system to change their password |
| **Logout URL** | Redirects customers to the external system on sign-out |

<Note>
When SSO is configured with external URLs for email or password changes, the corresponding fields in the shop's profile editor redirect to the external system rather than handling the change locally. This ensures the external identity provider remains the single source of truth.
</Note>
...
- [SSO](/core-platform/system-access/sso) -- Single Sign-On configuration
