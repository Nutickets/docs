...
<Tip>**Dynamic pricing uplift** added in Release 47. This preset is only available when [dynamic pricing](/core-platform/integrations/dynamic-pricing) is enabled for your account. It groups by event and defaults to the dynamic pricing revenue columns, pre-filtered to dynamically priced items.</Tip>
...
<Note>
The **Pricing type** grouping is only available when [dynamic pricing](/core-platform/integrations/dynamic-pricing) is enabled for your account.
</Note>
...
| **Failed scans report** | Failed scan attempts with failure details <Tip>New in Release 46</Tip> |
...
Default columns are zone, scan date, scanned barcode, scan status, status detail, customer email, and event details. All other scanning report fields can be added through [report customisation](/core-platform/reporting/fundamentals).
...
Provides a high-level overview of configured [schedules](/core-platform/event-management/schedules), including their settings and aggregate event counts.
...
Resale reports cover the [resale](/core-platform/resale/introduction) marketplace — the listings sellers create, the contracts that result when items resell, and the revenue from resold items. This category is available when resale is enabled for your account.
