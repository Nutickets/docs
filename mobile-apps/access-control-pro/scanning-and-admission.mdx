...
This is driven by the event's default **Entry** zone. When an admin sets the [Entry zone](/core-platform/event-management/access-control#the-default-entry-zone) to **In/Out Manual**, a **Scan in / Scan out** selector appears at the top-left of the Scan tab. Choose the direction before scanning:
...
Once synced to the platform, failed scans appear in the [Failed scans report](/core-platform/reporting/default-reports#scanning-reports) in admin, with the barcode, scan time, zone, device user, and the reason the scan was rejected.
...
The app decodes and validates the secure barcode locally before processing the admission. See [secure barcodes](/core-platform/event-management/secure-barcodes) for how this feature is configured.
