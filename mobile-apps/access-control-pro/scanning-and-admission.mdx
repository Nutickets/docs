...
The **Scan** tab is where operators spend most of their time. It supports four input methods — camera, laser scanner, manual entry, and NFC — and evaluates every scan instantly against locally cached ticket data. This page covers how scanning works, what the different results mean, and the options available for admitting attendees.
...
When **Auto admit** is enabled in settings, valid tickets are admitted automatically after scanning — no tap required. This is designed for high-throughput entry points where speed is critical.
