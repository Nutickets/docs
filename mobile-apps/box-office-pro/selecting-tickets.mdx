...
Adjust quantities using the on-screen controls. The app enforces any configured quantity rules:
...
On phones, the quantity control is a single button that opens a quantity entry sheet — tap it and type the number of tickets. Turn on **Quantity UI** in [settings](/mobile-apps/box-office-pro/settings-and-hardware#quantity-selection) to use inline **+** / **−** buttons instead. Tablets and larger terminals always show the **+** / **−** buttons. <Tip>Quantity UI setting added in Release 47</Tip>
...
<Tip>Zero-value orders skip payment from the ticket screen in Release 47</Tip>

<Note>
When the order total is zero — for example, all-complimentary tickets — tapping **Checkout** skips the payment screen entirely and takes you straight to the payment complete screen, with no payment method required. This matches the [basket checkout](#proceeding-to-checkout) behaviour below.
</Note>
