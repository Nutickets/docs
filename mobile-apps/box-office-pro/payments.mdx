...
Box Office Pro supports multiple payment methods that can be used individually or combined through split payments. After building a basket and tapping **Checkout**, the payment screen shows the total cost and the available payment methods. Each method can be enabled or disabled in [settings](/mobile-apps/box-office-pro/settings-and-hardware).
...
From the confirmation screen, several actions are available:
